...
public class ProjectDiscussionImple implements ProjectDiscussionService {
    ...
    public void deleteReply(Long replyId) {
        DiscussionReply reply = replyRepo.findById(replyId)
                .orElseThrow(() -> new RuntimeException("Reply not found"));
        ensureDiscussionProjectIsApproved(reply.getDiscussion());
        ensureCanModerate(reply.getDiscussion());
    ...
    private boolean isCurrentAuthor(UserSignUp author) {
        ...
        return current != null && author.getId()==current.getId();
    }

    private void ensureAuthor(UserSignUp author) {
        if (!isCurrentAuthor(author)) {
    ...
    private void ensureCanModerate(ProjectDiscussion discussion) {
        UserSignUp current = getCurrentAccount();
        if (isCurrentAuthor(discussion.getAuthor()) || "ROLE_ADMIN".equals(current.getRole())) {
            return;
        }
        if (discussion.getProject() != null && discussion.getProject().getStudent() != null && 
            current.getMail().equalsIgnoreCase(discussion.getProject().getStudent().getStudentEmail())) {
            return;
        }
        if (discussion.getGroupProject() != null && discussion.getGroupProject().getTeamLead() != null && 
            current.getMail().equalsIgnoreCase(discussion.getGroupProject().getTeamLead().getStudentEmail())) {
            return;
        }
        throw new RuntimeException("Not authorized");
    }
