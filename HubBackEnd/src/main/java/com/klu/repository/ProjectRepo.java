...
public interface ProjectRepo extends JpaRepository<Project,Integer>{
    ...
    List<Project> findByStatusAndStudentBranchAndStudentYear(
            ProjectStatus status, String branch, Integer year);
    ...
    List<Project> findApprovedProjectsByStudentIds(
    ...
    @Query("select p.choice, count(p) from Project p where p.status = com.klu.model.ProjectStatus.APPROVED group by p.choice order by count(p) desc")
