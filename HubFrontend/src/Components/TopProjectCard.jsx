      ...
      <div className="flex w-full flex-col rounded-2xl border border-slate-400 bg-white p-3 shadow-sm transition-all duration-300 hover:bg-cream hover:shadow-xl hover:border-amber-800 sm:p-4">
        ...
        <div className="flex cursor-pointer items-start gap-3 rounded-lg p-2 transition-all duration-200 hover:bg-tan/40 sm:gap-4" onClick={handleProfileClick}>
          ...
          <div className="flex-1">
            <p className="break-words text-sm font-semibold leading-none sm:text-base">{project.ownerName}</p>
            <p className="mt-1 text-xs text-slate-500 sm:text-sm">#{project.ownerId}</p>
            {project.type === "GROUP" && project.studentList?.length > 0 && (
              <div className="m-2 flex flex-wrap gap-2">
                {project.studentList.filter((student) => student.studentId !== project.ownerId).map((student) => (
                  <span key={student.studentId} onClick={(e) => { e.stopPropagation(); navigate(`/profile/${student.studentId}`); }} className="rounded-md bg-slate-200 px-2 py-0.5 text-[10px] text-slate-700 sm:text-xs">{student.student_name}</span>
                ))}
              </div>
            )}
            <p className="mt-3 text-sm font-medium text-bloodstone sm:text-base">{project.tech1} . {project.tech2}</p>
          </div>
        </div>
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-500 sm:justify-end sm:gap-5">
          <button onClick={handleLike} className={`flex items-center gap-1 transition ${isReviewer ? "cursor-default opacity-80" : "hover:text-red-500"}`}>
            ...
            <span className="font-semibold text-slate-700">{like}</span> Likes
          </button>

          <button type="button" onClick={() => setDiscussionOpen(true)} className="flex items-center gap-1 text-slate-600 hover:text-blue-600" aria-label="Open project discussion">
            <MessageCircle size={18} />
            <span className="font-semibold">{project.discussionCount || 0}</span>
          </button>

          <a className="transition hover:text-black" href={project.githubUrl} target="_blank" rel="noopener noreferrer" onClick={handleGithubClick}><FaGithub className="text-2xl sm:text-3xl" /></a>
          {project.liveUrl && <a href={project.liveUrl} className="font-semibold text-blue-600 hover:underline" onClick={handleLiveDemoClick}>View</a>}
        </div>
      </div>
