...
const Card = ({ project }) => {
  ...
  return (
    <>
      <div className="relative flex h-full w-full flex-col overflow-hidden rounded-2xl border border-tan/40 bg-cover bg-center p-3 text-white sm:p-4" style={{ backgroundImage: `url(${bg})` }}>
        ...
        <div className="relative z-10">
          ...
          <div className="flex cursor-pointer items-start gap-2 rounded-lg p-2 transition hover:bg-light-blue/10 sm:gap-3" onClick={handleProfileClick}>
            <img
              src={project.type === "GROUP" ? (project.teamLead?.avatarUrl || `/avatars/Avatar${(project.teamLead?.studentId % 40) + 1}.webp`) : (project.student?.avatarUrl || `/avatars/Avatar${(project.student?.studentId % 40) + 1}.webp`)}
              className="h-12 w-12 flex-shrink-0 rounded-full border-2 border-white/30 object-cover sm:h-14 sm:w-14"
            />
            <div className="flex-1">
              <p className="break-words text-sm text-white sm:text-base">{project.ownerName} . {project.ownerId}</p>
              {project.type === "GROUP" && project.studentList?.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {project.studentList.filter((student) => student.studentId !== project.ownerId).map((student) => (
                    <span key={student.studentId} onClick={(e) => { e.stopPropagation(); navigate(`/profile/${student.studentId}`); }} className="rounded bg-tan/25 px-2 py-1 text-[10px] text-vanilla-custard sm:text-xs">
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-xs sm:justify-end sm:gap-6">
            <button onClick={handleLike} className={`flex items-center gap-1 ${isReviewer ? "cursor-default opacity-80" : "active:scale-95"}`}>
              Likes <FaHeart className={liked ? "text-red-500" : "text-gray-300"} /> <span>{like}</span>
            </button>

            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); setDiscussionOpen(true); }}
              className="flex items-center gap-1 text-vanilla-custard transition hover:text-white active:scale-95"
              aria-label="Open project discussion"
            >
              <MessageCircle size={19} />
              <span>{project.discussionCount || 0}</span>
            </button>

            <a className="transition hover:scale-110" href={project.githubUrl} target="_blank" rel="noopener noreferrer" onClick={handleGithubClick}>
              <FaGithub size={22} />
            </a>
            {project.liveUrl && (
              <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 rounded-2xl bg-sky-500 px-2 py-1 text-xs text-vanilla-custard transition hover:bg-sky-700 sm:px-3 sm:text-sm" onClick={handleLiveDemoClick}>
      ...
      <ProjectDiscussion project={project} isOpen={discussionOpen} onClose={() => setDiscussionOpen(false)} />
    </>
  );
};
