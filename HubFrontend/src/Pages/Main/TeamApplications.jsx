    ...
    <div className="min-h-screen bg-linear-to-br from-slate-950 via-blue-950 to-slate-950">
        ...
        <div className="relative min-h-65 md:h-65 rounded-b-[30px] md:rounded-b-[50px] rounded-tr-[30px] md:rounded-tr-[50px] overflow-hidden shadow-2xl">
          ...
          <div className="absolute inset-0 flex flex-col lg:flex-row justify-end lg:justify-between items-start lg:items-end px-5 sm:px-8 lg:px-10 pb-6 lg:pb-10 gap-4">
            ...
            <button onClick={() => navigate("/formATeam")}
                    className="w-full sm:w-auto mt-2 lg:mt-6 flex items-center justify-center gap-2 bg-white text-blue-700 px-6 py-3 rounded-full font-semibold shadow-lg hover:bg-blue-50 hover:scale-105 transition-all duration-200">
                    <Plus size={18} />
              Create Team
            </button>
          </div>
            ...
            {myTeams.map((team) => {
              ...
              return (
                <div key={team.collaboration_id} className="bg-white/5 border border-white/10 overflow-hidden backdrop-blur-xl">
                  <div onClick={() => setExpandedTeam(isOpen ? null : team.collaboration_id)} className="w-full flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 px-5 sm:px-8 py-5 text-left">
                    ...
                    <div className="border-t border-white/10 px-4 sm:px-8 py-5">
                      {apps.length === 0 ? (
                        ...
                        <div className="flex flex-col gap-3">
                          {apps.map((app) => (
                            <div key={app.collabApplication_id} onClick={() => navigate(`/profile/${app.student?.studentId}`)} 
                                  className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 bg-white/5 px-4 sm:px-5 py-4 cursor-pointer hover:bg-white/10 transition">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-16">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-white text-xl sm:text-2xl font-bold">Explore Teams</h2>
          <span className="text-slate-400 text-sm mt-0.5">— find your next collaboration</span>
        </div>

        {allTeams.length === 0 ? (
          <div className="bg-white/5 border text-white border-white/10 p-4 sm:p-6 flex flex-col gap-4 backdrop-blur-xl hover:-translate-y-1 transition-all duration-300">No teams available right now to apply.</div>
        ) : (
          ...
          {allTeams.map((team) => {
            ...
            return (
              <div key={team.collaboration_id} className="bg-white/5 border border-white/10 p-6 flex flex-col gap-4 backdrop-blur-xl hover:-translate-y-1 transition-all duration-300">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mt-auto pt-2 border-t border-white/10">
                  {team.linkedIn ? (
