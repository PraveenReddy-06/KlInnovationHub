...
const ExploreProjects = () => {
  ...
  const filteredProjects = useMemo(() => {
    return allProjects.filter((project) => {
      ...
      const branch = isGroup ? "" : project.student?.branch;
      const year = isGroup ? null : project.student?.year;
            ...
            <>
              <div className="grid grid-cols-4 gap-5">
               {paginatedProjects.map((project) => {
                  ...
                  const ownerName = isGroup ? "Team Project" : project.student?.student_name;
                  const ownerId = isGroup ? "GROUP" : project.student?.studentId;
                  const branch = isGroup ? "MULTI" : project.student?.branch;
                  const year = isGroup ? "-" : project.student?.year;
                    ...
                    <div key={`${project.type}-${project.projectId || project.groupProjectId}`} className="bg-white rounded-xl shadow hover:shadow-lg transition overflow-hidden">
                      <div className="p-4">
                        ...
                        <div className="flex gap-2 mt-4">
                          ...
                          <a href={isGroup ? project.github_url : project.githubUrl} target="_blank" rel="noreferrer" className="border px-3 rounded flex items-center justify-center hover:bg-gray-100"><ExternalLink size={18}/></a>
                        </div>
                      </div>
                    </div>
                  );
